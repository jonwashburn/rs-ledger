...
theorem all_predictions_positive_uncertainty :
  ...
  -- This follows from the fundamental uncertainty in E_coh measurements
  cases' p with value uncertainty category rung validated
  simp
  -- Uncertainty is always positive due to measurement limitations
  -- and the quantum nature of the coherence energy scale
  norm_num
...
theorem particle_masses_on_phi_ladder :
  ...
  intro p hp
-- All particle mass predictions in Recognition Science are derived from the phi ladder
-- Each particle corresponds to a specific rung n where mass = E_coh * φ^n
unfold particleMassPredictions at hp
-- The construction of particle mass predictions assigns each a definite rung
simp at hp
cases hp with
| _ => simp [Option.isSome]
